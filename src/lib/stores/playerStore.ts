    ...
    update((state) => {
      ...
      if (newProgress >= 100) {
        return {
          ...
          progress: 100,
          showNextPreview: false,
          peekLatched: state.peekLatched || showNextPreview,
        };
      }
  ...
  const api = {
    ...
    setOptimisticSeek(progressPct: number, ttlMs = 5000) {
      ...
      update((state) => {
        ...
        optimisticSeek =
          ttlMs > 0
            ? { value: pct, until, trackId: state.currentTrack?.id ?? null }
            : null;
        return {
          ...state,
          progress: pct,
    ...
    setPlaybackState(args: {
      ...
      const allowProgressFromServer = () => {
        ...
        if (
          optimisticSeek.trackId &&
          incomingTrackId &&
          optimisticSeek.trackId !== incomingTrackId
        ) {
          optimisticSeek = null;
          return true;
        }
        ...
        const serverClose =
          Math.abs((args.progressPct ?? 0) - optimisticSeek.value) <= 1.25;
        if (serverClose) {
          optimisticSeek = null;
          return true;
        }
      ...
      update((state) => {
        ...
        const effectiveTrack = useServerTrack
          ? args.current
          : state.currentTrack;
        ...
        const suppressForPeek = !!(
          changedTrack &&
          lastPeekTrackId &&
          nextId &&
          lastPeekTrackId === nextId
        );
        toastTrack = changedTrack && !suppressForPeek ? args.current : null;

        const nextProgress =
          useServerTrack && allowProgressFromServer()
            ? clamp(args.progressPct, 0, 100)
            : state.progress;

        const trackForCalc = effectiveTrack ?? args.current;
        const remainingMs = trackForCalc
          ? trackForCalc.duration * (1 - nextProgress / 100)
          : Infinity;
        const showNextPreview = remainingMs <= 15000 && !!effectiveNext;

        const peekLatched =
          showNextPreview || (state.peekLatched && args.progressPct >= 99.5);
    ...
    setQueueLoading(loading: boolean) {
      update((state) => ({ ...state, isQueueLoading: loading }));
    },
...
export const formattedProgress = derived(playerStore, ($player) => {
  const currentMs =
    ($player.currentTrack?.duration ?? 0) * ($player.progress / 100);
