...
>
  {#if $playerStore.currentTrack}
    <div
      class={`w-full grid gap-x-5 gap-y-3 ${$playerStore.expanded ? 'grid-cols-[auto,minmax(0,1fr)] items-start' : 'grid-cols-[auto,minmax(0,1fr),auto] items-center'}`}
    >
      <div class={`relative shrink-0 ${$playerStore.expanded ? '' : 'row-span-2'}`}>
        <img
          data-nowplaying-cover
          src={$playerStore.currentTrack.albumArt}
          alt={$playerStore.currentTrack.album}
          class={`${$playerStore.expanded ? 'w-56 h-56 rounded-3xl' : 'w-20 h-20 rounded-2xl'} object-cover shadow-xl transition-all duration-300 ease-out`}
        />
      ...
      <div class={`min-w-0 ${$playerStore.expanded ? 'pt-2' : ''}`}>
        {#if $playerStore.expanded}
      ...
      <div
        class={`${$playerStore.expanded ? 'col-span-2' : 'col-start-2 col-span-2'} flex items-center w-full`}
      >
        <input
          bind:this={seekEl}
          type="range"
