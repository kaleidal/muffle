<script lang="ts">
    ...
    $: if (
        variant === "toast" &&
        toastActive &&
        $playerStore.nowPlayingToastKey !== lastToastKey
    ) {
        ...
        void (async () => {
            ...
            toastEl.animate(
                [
                    ...
                    { ...shown, offset: 0.18 },
                    {
                        transform: `translate(${dx - pillToImgCx}px, ${dy - pillToImgCy}px) scale(${scale})`,
                        opacity: 0,
                    },
                ],
                {
    ...
    $: if (
        variant === "peek" &&
        !latched &&
        $playerStore.showNextPreview &&
        $playerStore.nextTrack
    ) {
        latched = {
            albumArt: $playerStore.nextTrack.albumArt,
            album: $playerStore.nextTrack.album,
        };
    }
    ...
    $: active =
        variant !== "peek"
            ? null
            : (latched ??
              (($playerStore.peekLatched || $playerStore.showNextPreview) &&
              $playerStore.nextTrack
                  ? {
                        albumArt: $playerStore.nextTrack.albumArt,
                        album: $playerStore.nextTrack.album,
        ...
        void (async () => {
            ...
            peekEl.animate(
                [
                    ...
                    {
                        transform: `translate(${dx * 0.85}px, ${dy * 0.85}px) scale(${Math.max(0.9, scale * 1.06)})`,
                        opacity: 1,
                        offset: 0.75,
                    },
                    {
...
{#if variant === "peek" && active}
    <div
        bind:this={peekEl}
        class="peek bg-[#141414] rounded-[40px] p-5 w-fit h-fit flex items-center justify-center relative {$playerStore.peekLatched ||
        $playerStore.showNextPreview ||
        $playerStore.isTransitioning
            ? 'visible'
            : ''} {$playerStore.isTransitioning ? 'clicking' : ''}"
    >
        <div class="relative">
            <img
                bind:this={imgEl}
            ...
            />
            <div
                class="absolute -left-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/10 backdrop-blur flex items-center justify-center"
            >
                <svg
...
{#if variant === "toast" && toastActive}
    <div
        bind:this={toastEl}
        class="toast bg-[#141414] rounded-[40px] p-5 w-fit h-fit flex items-center justify-center pointer-events-none"
    >
        <div class="relative">
            <img
                bind:this={toastImgEl}
                src={toastActive.albumArt}
                alt={toastActive.name}
                class="w-20 h-20 rounded-[28px] object-cover shadow-xl"
            />
            <div
                class="absolute -left-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-white/10 backdrop-blur flex items-center justify-center"
