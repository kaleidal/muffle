<script lang="ts">
    ...
    async function moveTrack(fromIndex: number, toIndex: number) {
        ...
        if (
            from < 0 ||
            to < 0 ||
            from >= data.tracks.length ||
            to >= data.tracks.length
        )
            return;
        if (from === to) return;
        ...
        busy = true;
        try {
            const nextSnapshot = await spotifyStore.reorderPlaylistTrack(
                data.id,
                from,
                to,
                data.snapshotId,
...
>
    {#if loading}
        <div
            class="flex-1 flex items-center justify-center text-white/50 text-sm font-semibold"
        >
            Loading…
        </div>
    {:else if error}
    ...
    {:else if data}
        <div class="flex items-center gap-6 shrink-0">
            <div class="relative shrink-0 overflow-visible">
                ...
                <button
                    class="absolute -right-2 -bottom-2 w-11 h-11 rounded-full bg-(--accent-primary) flex items-center justify-center bouncy-btn shadow-lg transition-opacity"
                    class:opacity-70={playLoading}
                    aria-label="Play"
                    onclick={playPlaylist}
                    disabled={busy || playLoading}
                >
                    {#if playLoading}
                        <div
            ...
            <div class="min-w-0">
                ...
                >
                    {playlistId === "liked" ? "Library" : "Playlist"}
                </p>
                <div class="flex items-center gap-4">
                    <h2
                        class="text-white text-5xl font-extrabold tracking-tight leading-none truncate flex-1 min-w-0"
                    >
                        {data.name}
                    </h2>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        {#if playlistId !== "liked"}
                            ...
                            >
                                ...
                                >
                                    <path
                                        d="M21 19V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2zM8.5 13.5 11 16l3.5-4.5L19 17H5l3.5-3.5z"
                                    />
                                </svg>
                            </button>
                        {/if}
                ...
                <p class="text-white/50 text-sm font-semibold mt-2 truncate">
                    {playlistId === "liked" ? "Saved tracks" : data.ownerName}
                </p>
            </div>
        </div>
        ...
        <div class="mt-6 flex-1 min-h-0 overflow-y-auto scrollbar-hide">
            <div class="flex flex-col">
                {#each data.tracks as t, i (`${t.uri}-${i}`)}
                    ...
                    >
                        <div
                            class="w-10 text-white/40 text-sm font-semibold tabular-nums"
                        >
                            {i + 1}
                        </div>
                        ...
                        <div
                            class="text-white/40 text-sm font-semibold tabular-nums shrink-0"
                        >
                        ...
                        {#if playlistId !== "liked"}
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        <path
                                            d="M7.41 9.41 12 14l4.59-4.59L18 10.83l-6 6-6-6z"
                                        />
                                    </svg>
                                </button>
                            </div>
