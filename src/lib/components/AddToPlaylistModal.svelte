<script lang="ts">
    ...
    let addingToId: string | null = null;
    ...
    async function addToLiked(force = false) {
        ...
        try {
            ...
            // Update the contains state to show it's now added
            containsById = { ...containsById, liked: true };
            successMessage = "Added to Liked Songs";
            // Auto-close after brief delay to show success
            setTimeout(() => {
                if (successMessage) close();
            }, 800);
        } catch (e: any) {
    ...
    async function addToPlaylist(
        ...
        busy = true;
        addingToId = playlistId;
        error = null;
        successMessage = null;
        try {
            await spotifyStore.addTrackToPlaylist(playlistId, track.uri);
            // Update the contains state to show it's now added
            containsById = { ...containsById, [playlistId]: true };
            successMessage = `Added to ${playlistName}`;
            // Auto-close after brief delay to show success
            setTimeout(() => {
                if (successMessage) close();
            }, 800);
        ...
        >
            ...
            >
                <div class="flex items-center gap-4">
                    <div
                        class="w-12 h-12 rounded-2xl overflow-hidden bg-white/5 shrink-0"
                    >
                        {#if track.albumArt}
                            <img
                                src={track.albumArt}
                    ...
                    <div class="min-w-0 flex-1">
                        <div
                            class="text-white font-extrabold tracking-tight truncate"
                        >
                            Add to playlist
                        </div>
                        <div
                            class="text-white/50 text-sm font-semibold truncate"
                        >
                    ...
                    >
                        ...
                        >
                            <path
                                d="M18.3 5.71 12 12.01l6.3 6.29-1.41 1.42L10.59 13.4 4.29 19.72 2.88 18.3 9.18 12 2.88 5.7 4.3 4.29l6.29 6.3 6.3-6.3z"
                            />
                        </svg>
                    </button>
                </div>
                ...
                {#if error}
                    <div class="mt-3 text-red-400/80 text-sm font-semibold">
                        {error}
                    </div>
                {/if}
                ...
                {#if successMessage}
                    <div
                        class="mt-3 flex items-center gap-2 text-green-400/90 text-sm font-semibold"
                    >
                        <svg
                            class="w-4 h-4"
                            fill="currentColor"
                ...
                {#if confirmTarget}
                    <div class="mt-4 rounded-3xl bg-white/5 p-4">
                        ...
                        <div class="mt-1 text-white/60 text-sm font-semibold">
                            This track is already in <span class="text-white/90"
                                >{confirmTarget.name}</span
                            >. Add it again?
                        </div>
                ...
                {:else}
                    <div class="mt-4 max-h-72 overflow-y-auto scrollbar-hide">
                        <button
                            type="button"
                            class="w-full text-left px-4 py-3 rounded-2xl transition-colors flex items-center justify-between {containsById[
                                'liked'
                            ] === true
                                ? 'bg-white/8'
                        ...
                        >
                            ...
                            {#if addingToId === "liked"}
                                <div
                                    class="loading-spinner w-4 h-4 border-2 border-white/20 border-t-white/60 rounded-full"
                                ></div>
                            {:else if checkingById["liked"]}
                                <div
                                    class="text-white/35 text-xs font-semibold"
                                >
                                    Checking…
                                </div>
                            {:else if containsById["liked"] === true}
                                <div
                                    class="flex items-center gap-1 text-green-400/80 text-xs font-extrabold tracking-tight"
                                >
                                    <svg
                                        class="w-3 h-3"
                                        fill="currentColor"
                        ...
                        <div class="mt-3 flex flex-col gap-2">
                            {#each visiblePlaylists as p (p.id)}
                                <button
                                    type="button"
                                    class="w-full text-left px-4 py-3 rounded-2xl transition-colors flex items-center gap-3 {containsById[
                                        p.id
                                    ] === true
                                        ? 'bg-white/8'
                                        : 'bg-white/5 hover:bg-white/10'}"
                                    onclick={() =>
                                        addToPlaylist(p.id, p.name, false)}
                                    disabled={busy}
                                >
                                    <div
                                        class="w-10 h-10 rounded-xl overflow-hidden bg-white/5 shrink-0"
                                    >
                                        {#if p.images?.[0]?.url}
                                            <img
                                                src={p.images[0].url}
                                    ...
                                    {#if addingToId === p.id}
                                        <div
                                            class="loading-spinner w-4 h-4 border-2 border-white/20 border-t-white/60 rounded-full shrink-0"
                                        ></div>
                                    {:else if checkingById[p.id]}
                                        <div
                                            class="text-white/35 text-xs font-semibold shrink-0"
                                        >
                                            Checking…
                                        </div>
                                    {:else if containsById[p.id] === true}
                                        <div
                                            class="flex items-center gap-1 text-green-400/80 text-xs font-extrabold tracking-tight shrink-0"
                                        >
                                            <svg
                                                class="w-3 h-3"
                                                fill="currentColor"
                            ...
                            {#if $spotifyStore.status === "authenticated" && visiblePlaylists.length === 0}
                                <div
                                    class="px-2 py-4 text-white/45 text-sm font-semibold"
                                >
                                    No playlists found
                                </div>
                            {/if}
