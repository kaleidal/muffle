...
function startLibrespot(accessToken) {
  ...
  if (!fs.existsSync(binPath)) {
    ...
    console.warn(
      "Muffle will work in Connect-only mode (control external devices)",
    );
    return;
  }
...
async function exchangeSpotifyCode(args) {
  console.log("[main] Exchanging authorization code for tokens...");
  ...
  if (!res.ok) {
    console.error(
      "[main] Token exchange failed:",
      json?.error,
      json?.error_description,
    );
    throw new Error(
      json?.error_description || json?.error || "Spotify token exchange failed",
    );
  }

  console.log("[main] Token exchange successful");
  console.log("[main] Got refresh token:", !!json.refresh_token);
  console.log("[main] Token expires in:", json.expires_in, "seconds");

  if (!json.refresh_token) {
    console.warn("[main] WARNING: No refresh token received from Spotify!");
  }
...
async function refreshSpotifyToken(args) {
  console.log("[main] Attempting to refresh Spotify token...");

  if (!args.refreshToken) {
    console.error("[main] No refresh token provided!");
  ...
  console.log(
    "[main] Token refresh successful, expires in",
    json.expires_in,
    "seconds",
  );
  ...
  if (hasRefreshToken) {
    console.log("[main] Spotify returned a new refresh token");
  }
  ...
  spotifyAuthServer = http.createServer(async (req, res) => {
    try {
      ...
      if (error) {
        ...
        res.end(
          `<html><body><h2>Spotify login failed</h2><p>${error}</p><p>You can close this window.</p></body></html>`,
        );
        if (mainWindow) {
...
app.whenReady().then(async () => {
  ...
  const csp = [
    ...
    isDev
      ? `img-src 'self' data: https: ${devOrigin}`
...
ipcMain.handle("spotify:refresh", async (_event, args) => {
  console.log("[main] IPC: spotify:refresh called");
  try {
    const result = await refreshSpotifyToken(args);
    console.log("[main] IPC: spotify:refresh succeeded");
    return result;
  } catch (e) {
    const message = e?.message || "Spotify token refresh failed";
    console.error("[main] IPC: spotify:refresh failed:", message);
    // Don't send auth-error for refresh failures - let the renderer handle retry logic
    // Only send auth-error for actual revocation/invalid grant
    if (/revoked|invalid_grant|invalid_client/i.test(message)) {
      if (mainWindow) {
